...
double CustomizedLoadBalancer::hostWeight(const Host& host) const {
...
HostConstSharedPtr CustomizedLoadBalancer::unweightedHostPeek(const HostVector&,
                                                                const HostsSource&) {
  // CustomizedLoadBalancer can not do deterministic preconnecting, because
  // any other thread might select the least-requested-host between preconnect and
  // host-pick, and change the rq_active checks.
  return nullptr;
}

HostConstSharedPtr CustomizedLoadBalancer::unweightedHostPick(const HostVector& hosts_to_use,
                                                                const HostsSource&) {
  HostSharedPtr candidate_host = nullptr;

  switch (selection_method_) {
  case envoy::extensions::load_balancing_policies::customized::v3::Customized::FULL_SCAN:
    candidate_host = unweightedHostPickFullScan(hosts_to_use);
    break;
  case envoy::extensions::load_balancing_policies::customized::v3::Customized::N_CHOICES:
...
HostSharedPtr CustomizedLoadBalancer::unweightedHostPickFullScan(const HostVector& hosts_to_use) {
  HostSharedPtr candidate_host = nullptr;
...
HostSharedPtr CustomizedLoadBalancer::unweightedHostPickNChoices(const HostVector& hosts_to_use) {
  HostSharedPtr candidate_host = nullptr;
