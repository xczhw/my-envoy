 ...
 */
class CustomizedLoadBalancer : public EdfLoadBalancerBase {
public:
  CustomizedLoadBalancer(
      const PrioritySet& priority_set, const PrioritySet* local_priority_set, ClusterLbStats& stats,
      Runtime::Loader& runtime, Random::RandomGenerator& random,
      const envoy::config::cluster::v3::Cluster::CommonLbConfig& common_config,
      OptRef<const envoy::config::cluster::v3::Cluster::CustomizedLbConfig> customized_config,
      ...
      : EdfLoadBalancerBase(
            ...
            customized_config.has_value()
                ? LoadBalancerConfigHelper::slowStartConfigFromLegacyProto(
                      customized_config.ref())
                : absl::nullopt,
            time_source),
        choice_count_(
            customized_config.has_value()
                ? PROTOBUF_GET_WRAPPED_OR_DEFAULT(customized_config.ref(), choice_count, 2)
                : 2),
        active_request_bias_runtime_(
            customized_config.has_value() && customized_config->has_active_request_bias()
  ...
  CustomizedLoadBalancer(
      ...
      const envoy::extensions::load_balancing_policies::customized::v3::Customized&
          customized_config,
      TimeSource& time_source)
      : EdfLoadBalancerBase(
            priority_set, local_priority_set, stats, runtime, random, healthy_panic_threshold,
            LoadBalancerConfigHelper::localityLbConfigFromProto(customized_config),
            LoadBalancerConfigHelper::slowStartConfigFromProto(customized_config), time_source),
        choice_count_(PROTOBUF_GET_WRAPPED_OR_DEFAULT(customized_config, choice_count, 2)),
        active_request_bias_runtime_(
            customized_config.has_active_request_bias()
                ? absl::optional<Runtime::Double>(
                      {customized_config.active_request_bias(), runtime})
                : absl::nullopt),
...
private:
  ...
  // performance reasons and refresh it in `CustomizedLoadBalancer::refresh(uint32_t priority)`
  // whenever a `HostSet` is updated.
  double active_request_bias_{};

  const absl::optional<Runtime::Double> active_request_bias_runtime_;
  const envoy::extensions::load_balancing_policies::customized::v3::Customized::SelectionMethod
      selection_method_{};
};
